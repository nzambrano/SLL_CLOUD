...
public interface M4sllFasesRepository extends JpaRepository<M4sllFases, M4sllFasesId> {
    ...
    @Query(
        value = "SELECT * FROM sll.m4sll_fases WHERE id_organization = :idOrganization AND lit_id_litigio = :litIdLitigio AND tfa_id_tp_fase = :tfaIdTpFase ORDER BY id_organization, lit_id_litigio, tfa_id_tp_fase DESC",
        nativeQuery = true
    )
    ...
    @Query(
        value = "SELECT * FROM sll.m4sll_fases WHERE id_organization = :idOrganization AND lit_id_litigio = :litIdLitigio ORDER BY id_organization, lit_id_litigio, tfa_id_tp_fase DESC",
        nativeQuery = true
    )
