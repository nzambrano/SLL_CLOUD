...
public interface StdSubGeoDivRepository extends JpaRepository<StdSubGeoDiv, StdSubGeoDivId> {
    @Query(
        value = "select COALESCE(max(cast(std_id_sub_geo_div as integer)),0)+1 from public.std_sub_geo_div where id_organization = :id_organization and std_id_country = :std_id_country and std_id_geo_div = :std_id_geo_div",
        nativeQuery = true
    )
    ...
    @Query(
        value = "select * from public.std_sub_geo_div where id_organization = :id_organization and std_id_country = :std_id_country and std_id_geo_div = :std_id_geo_div",
        nativeQuery = true
    )
