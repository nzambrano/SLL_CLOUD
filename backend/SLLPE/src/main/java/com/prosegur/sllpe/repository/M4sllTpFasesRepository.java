...
public interface M4sllTpFasesRepository extends JpaRepository<M4sllTpFases, M4sllTpFasesId> {
	...
	@Query(value = "select COALESCE(cast( max(tfa_id_tp_fase) as integer),0)+1 as secuencia from sll.m4sll_tp_fases where id_organization = :id_organization", nativeQuery = true)
