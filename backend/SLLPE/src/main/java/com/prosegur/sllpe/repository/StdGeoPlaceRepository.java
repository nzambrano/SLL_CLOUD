...
public interface StdGeoPlaceRepository extends JpaRepository<StdGeoPlace, StdGeoPlaceId> {
    @Query(
        value = "select COALESCE(max(cast(std_id_geo_place as integer)),0)+1 from sll.std_geo_place where id_organization = :id_organization and std_id_country = :std_id_country and std_id_geo_div = :std_id_geo_div and std_id_sub_geo_div = :std_id_sub_geo_div",
        nativeQuery = true
    )
    ...
    @Query(
        value = "select * from sll.std_geo_place where id_organization = :id_organization and std_id_country = :std_id_country and std_id_geo_div = :std_id_geo_div and std_id_sub_geo_div = :std_id_sub_geo_div",
        nativeQuery = true
    )
